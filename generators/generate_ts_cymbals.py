...
# Process each cymbal type and variant
for cymbal_type in sorted(os.listdir(ts_cymbals_path)):
    cymbal_type_path = os.path.join(ts_cymbals_path, cymbal_type)
    if not os.path.isdir(cymbal_type_path):
        continue
    
    for variant in sorted(os.listdir(cymbal_type_path)):
        variant_path = os.path.join(cymbal_type_path, variant)
        if not os.path.isdir(variant_path):
            continue
        
        cymbal_name = f"{cymbal_type} {variant}"
        print(f"Processing {cymbal_name}")
        
        root_folder = variant_path
        
        # Collect samples by mic and velocity
        samples_by_mic_vel = {}
        for mic in mic_to_slot.keys():
            mic_path = os.path.join(variant_path, mic)
